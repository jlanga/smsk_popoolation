...
#--gtf /Users/robertkofler/testfiles/3R-3entries.gtf --input /Users/robertkofler/testfiles/3R.pileup --output /Users/robertkofler/testfiles/output/3R-filtered.pileup
    ...
    pod2usage( -msg => "Could not find sam file", -verbose => 1 )
        unless -e $input;
...

{
    ...
    sub mask_sequence {
        ...
        # masking and control
        my $maskedseq
            = Utility::_mask_sequence_from_indels( $seq, $indelpositions,
            $maskregion );
        my $maskleng = length($maskedseq);
        die "sequence after masking does not have its original length"
            unless $maskleng == $orileng;
