...
pod2usage(
    -msg     => "Please provide a valid target coverage (>0)",
    -verbose => 1
) unless $targetcoverage;
pod2usage(
    -msg     => "Please provide a valid maximum coverage (>0)",
    -verbose => 1
) unless $maxcoverage;
pod2usage( -msg => "Please provide a valid method", -verbose => 1 )
    unless $method;
...
my $pp = get_pileup_parser( $encoding, 1, $targetcoverage, $maxcoverage,
    $minqual );
...
# $p=pos, chr, refc, totcov, eucov, A, T, C, G, del, N, iscov, issnp, ispuresnp
    ...
    my @toprint = (
        $p->{chr}, $p->{pos}, $p->{refc}, length($newnucs), $newnucs, $newqual
    );
...
# calculate the new frequencies using the preset frequency calculator (random sampling method)
            my $toret = $freqcalculator->(
                $targetcoverage, $p->{A}, $p->{T}, $p->{C}, $p->{G},
                $p->{del},       $p->{N}
            );
            return $toret;
        }
    ...
    sub _freq_withoutreplacement {
        ...
        my $temp
            ...
            . "N" x $n;
        my $cov = length($temp);
        die "Coverage $cov smaller than targetcoverage $targetcoverage"
    ...
    sub _get_fraction_sample {
        ...
        my ( $an, $tn, $cn, $gn, $deln, $nn ) = (
            _intround( $af * $itercoverage ),
            _intround( $tf * $itercoverage ),
            _intround( $cf * $itercoverage ),
            _intround( $gf * $itercoverage ),
            _intround( $delf * $itercoverage ),
            _intround( $nf * $itercoverage )
        );
...
# ingenious first increase the coverage, than try to decrease it, in case it is to small fill it up with N
        ...
        my $str
            ...
            . "N" x $nn;
        die
            "New string is longer than targeted $str vs $targetcoverage; Use random sampling instead to obtain uniform coverages"
            if ( length($str) > $targetcoverage );
