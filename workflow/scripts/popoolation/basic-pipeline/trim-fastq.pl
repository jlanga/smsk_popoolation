...
#--input /Volumes/Volume_3/analysis/clipsignal/raw/s100000.fastq --output /Users/robertkofler/tmp/filter.test.fastq
    ...
    pod2usage( -msg => "Quality must be larger than 0", -verbose => 1 )
        if $qualThreshold < 0;
    pod2usage( -msg => "Length must be larger than 0", -verbose => 1 )
        if $minLength < 1;    # min length has to be 1 or larger
    pod2usage(
        -msg     => "At least one input file has to be provided",
        -verbose => 1
    ) unless -e $input1;
    pod2usage( -msg => "An output file has to be provided", -verbose => 1 )
        unless $output;
    ...
    if ( -e $input2 ) {
        print
            "Found an existing file for the second read; Switching to paired-read mode\n";

        MainProc::processPE(
            $input1,      $input2,      $output,
            $encoder,     $trimQuality, $qualThreshold,
            $processStep, $minLength,   $discardRemainingNs,
            $verbose
        );
    }
    else {
        print
            "Did not find an existing file for the second read; Switching to single-read mode\n";
        MainProc::processSE(
            $input1,      $output,             $encoder,
            $trimQuality, $qualThreshold,      $processStep,
            $minLength,   $discardRemainingNs, $verbose
        );
...

{
    ...
    sub processSE {
        ...
        my ( $count5ptrims, $count3ptrims, $countRemainingNdiscards,
            $countQualityTrims, $countLengthDiscard )
            = ( 0, 0, 0, 0, 0 );
        my $countProcessed    = 0;
        ...
        while (1) {
            my ( $firstheader, $nucleotide, $secondheader, $quality )
                = $fastqr->nextRead();
            last unless $firstheader;
            die
                "Error in sequence $firstheader; nucleotide sequence and quality sequence do not have equal length $nucleotide - $quality\n"
            ...
            ( $nucleotide, $quality, $c5ptrims, $c3ptrims )
                = Utility::trimNs( $nucleotide, $quality );
            ...
            if ($trimQuality) {
                ...
                my $ctrimqual = 0;
                ( $nucleotide, $quality, $ctrimqual )
                    = Utility::trimQualityMott( $nucleotide, $quality,
                    $qualThreshold, $encoder );
            ...
            $countPasFiltering++;
            Utility::printFastq( $ofh, $firstheader, $nucleotide,
                $secondheader, $quality );
        }
        ...
        if ($verbose) {
            ...
            print
                "Reads discarded during 'remaining N filtering': $countRemainingNdiscards\n";
            print
                "Reads discarded during length filtering: $countLengthDiscard\n";
            print
                "Count sequences trimed during quality filtering: $countQualityTrims\n"
                if $trimQuality;
    ...
    sub processPE {
        ...
        my ( $count5ptr1, $count3ptr1, $countRemainingNdiscards1,
            $countQualityTrims1, $countLengthDiscard1 )
            = ( 0, 0, 0, 0, 0 );
        my ( $count5ptr2, $count3ptr2, $countRemainingNdiscards2,
            $countQualityTrims2, $countLengthDiscard2 )
            = ( 0, 0, 0, 0, 0 );
        my ( $paired, $single, $read1Passing, $read2Passing )
            = ( 0, 0, 0, 0 );
        ...
        while (1) {
            ...
            if (   ( $fheader1 and not $fheader2 )
                or ( $fheader2 and not $fheader1 ) )
            {
            ...
            die
                "Error in sequence $fheader1; nucleotide sequence and quality sequence do not have equal length $nuc1 - $qual1\n"
                unless length($nuc1) == length($qual1);
            die
                "Error in sequence $fheader2; nucleotide sequence and quality sequence do not have equal length $nuc2 - $qual2\n"
            ...
            ( $nuc1, $qual1, $c5ptr1, $c3ptr1 )
                = Utility::trimNs( $nuc1, $qual1 );
            $count5ptr1 += $c5ptr1;
            $count3ptr1 += $c3ptr1;
            ( $nuc2, $qual2, $c5ptr2, $c3ptr2 )
                = Utility::trimNs( $nuc2, $qual2 );
            $count5ptr2 += $c5ptr2;
            $count3ptr2 += $c3ptr2;
            ...
            if ($trimQuality) {
                ...
                ( $nuc1, $qual1, $ctrimqual1 )
                    = Utility::trimQualityMott( $nuc1, $qual1,
                    $qualThreshold, $encoder )
                    if $keep1 and $nuc1;
                $countQualityTrims1 += $ctrimqual1;

                ( $nuc2, $qual2, $ctrimqual2 )
                    = Utility::trimQualityMott( $nuc2, $qual2,
                    $qualThreshold, $encoder )
                    if $keep2 and $nuc2;
            ...
            if ( $keep1 and $keep2 ) {
                $paired++;
                Utility::printFastq( $ofh1, $fheader1, $nuc1, $sheader1,
                    $qual1 );
                Utility::printFastq( $ofh2, $fheader2, $nuc2, $sheader2,
                    $qual2 );
            }
            elsif ( $keep1 or $keep2 ) {
                $single++;
                if ($keep1) {
                    Utility::printFastq( $ofhse, $fheader1, $nuc1, $sheader1,
                        $qual1 );
                }
                elsif ($keep2) {
                    Utility::printFastq( $ofhse, $fheader2, $nuc2, $sheader2,
                        $qual2 );
                }
                else {
                    die "impossible";
...
#my($count5ptr1,$count3ptr1,$countRemainingNdiscards1,$countQualityTrims1,$countLengthDiscard1) = (0,0,0,0,0);
            ...
            print
                "Reads discarded during 'remaining N filtering': $countRemainingNdiscards1\n";
            print
                "Reads discarded during length filtering: $countLengthDiscard1\n";
            print
                "Count sequences trimed during quality filtering: $countQualityTrims1\n"
                if $trimQuality;
            ...
            print
                "Reads discarded during 'remaining N filtering': $countRemainingNdiscards2\n";
            print
                "Reads discarded during length filtering: $countLengthDiscard2\n";
            print
                "Count sequences trimed during quality filtering: $countQualityTrims2\n"
                if $trimQuality;
...

{
    ...
    sub trimNs {
        ...
        die "length of quality and nucleotide sequence must be equal"
            if length($nuc) != length($qual);
    ...
    sub trimQuality {
        ...
        my $totrim
            = _getQualityCutoff( $encoder, $qual, $minQual, $trimWindow );
        ...
        die
            "length of quality and nucleotide sequence must be equal $nuc vs $qual"
            if length($nuc) != length($qual);
    ...
    sub trimQualityMott {
        ...
        my $qual = shift;
        die
            "length of quality and nucleotide sequence has to be identical $nuc vs $qual"
            if length($nuc) != length($qual);
        my $minQual = shift;
        my $encoder = shift;
        ...
        return ( "", "", 1 )
            unless $qualhsp;    #return nothing if no hsp is found
        ...
        for ( my $i = 0; $i < @qual; $i++ ) {
            ...
            if ( $activescore > 0 ) {
                if ( $activescore > $highscore
                    ) # has to be larger than; larger or equal leads to inconsistent behaviour at the beginning of the quality hsp.
                {
...

{
    ...
    sub nextRead {
        ...
        die "first header must start with an @; line: $firstheader"
            unless $firstheader =~ m/^[@]/;
        die "second header must start with an +; line: $secondheader"
            unless $secondheader =~ m/^[+]/;
...

{
    ...
    sub testNTrimming {
        ...
        ( $t1, $t2, $c5, $c3 )
            = Utility::trimNs( "NNNNNAAANNNNN", "tttttxxxttttt" );
    ...
    sub testTrimQualityMott {
        ...
        is( $t1, "AAAA",
            "trim quality: min quality 20; AAAA with quality of TTTT not trimed"
        );
        is( $t2, "TTTT", "trim quality: quality was not trimmed either" );
        is( $c,  0,      "trim quality: trimmed flag set correctly" );

        ( $t1, $t2, $c )
            = Utility::trimQualityMott( "AAAAA", "UUUTS", 20, $t );
        is( $t1, "AAA",
            "trim quality: min quality 20; AAAAA with quality of UUUTS trimmed to AAA"
        );
        ...
        ( $t1, $t2, $c )
            = Utility::trimQualityMott( "TTAAAATT", "TTUUUUTT", 20, $t );
        is( $t1, "AAAA", "trim quality: sequence trimmed correctly" );
        is( $t2, "UUUU", "trim quality: quality was also trimmed correctly" );
        is( $c,  1,      "trim quality: trimmed flag set correctly" );

        ( $t1, $t2, $c )
            = Utility::trimQualityMott( "TTAAAATTCCCCC", "SSUUUUAAUUUUU", 20,
            $t );
        is( $t1, "CCCCC", "trim quality: sequence trimmed correctly" );
        is( $t2, "UUUUU",
            "trim quality: quality was also trimmed correctly" );
        is( $c, 1, "trim quality: trimmed flag set correctly" );

        ( $t1, $t2, $c )
            = Utility::trimQualityMott( "TTAAAATTCCCCCG", "SSUUUUAAUUUUUS",
            20, $t );
        is( $t1, "CCCCC", "trim quality: sequence trimmed correctly" );
        is( $t2, "UUUUU",
            "trim quality: quality was also trimmed correctly" );
        ...
        ( $t1, $t2, $c )
            = Utility::trimQualityMott( "TTAAAAATTCCCC", "SSUUUUUAAUUUU", 20,
            $t );
     ...
     # can a bases with a quality of 19 be skipped if the score is high enough
        ( $t1, $t2, $c )
            = Utility::trimQualityMott( "TTAAAAATTCCCC", "SSUUUTTSSUUUT", 20,
            $t );
        ...
        ( $t1, $t2, $c )
            = Utility::trimQualityMott( "TAAAAATTCCCCT", "SUUUTTSSUUUTS", 20,
            $t );
        is( $t1, "AAAAATTCCC", "trim quality: sequence trimmed correctly" );
        is( $t2, "UUUTTSSUUU",
            "trim quality: quality was also trimmed correctly" );
        ...
        ( $t1, $t2, $c )
            = Utility::trimQualityMott( "TAAAAATTCCCCTTTTT",
            "SUUUTTSSUUUTSSUTS", 20, $t );
        is( $t1, "AAAAATTCCC", "trim quality: sequence trimmed correctly" );
        is( $t2, "UUUTTSSUUU",
            "trim quality: quality was also trimmed correctly" );
        is( $c, 1, "trim quality: trimmed flag set correctly" );
        ...
        ( $t1, $t2, $c )
            = Utility::trimQualityMott( "TTTTTAAAAATTCCCCT",
            "UTTSSUUUTTSSUUUTS", 20, $t );
        is( $t1, "AAAAATTCCC", "trim quality: sequence trimmed correctly" );
        is( $t2, "UUUTTSSUUU",
            "trim quality: quality was also trimmed correctly" );
        is( $c, 1, "trim quality: trimmed flag set correctly" );
    ...
    sub testTrimQuality {
        ...
        ( $t1, $t2, $c )
            = Utility::trimQuality( "AAAAA", "TTTSS", 20, $t, 1 );
        is( $t1, "AAA",
            "trim quality: min quality 20; AAAAA with quality of TTTSS trimmed to AAA"
        );
        ...
        is( $t1, "",
            "trim quality: min quality 20; AAAA with quality of STTT empty" );
        is( $t2, "", "trim quality: quality string is empty as well" );

        # set quality neighborhood to three
        ( $t1, $t2, $c )
            = Utility::trimQuality( "AAAAAA", "TTTTSS", 20, $t, 3 );
        is( $t1, "AAA",
            "trim quality: min quality 20; window size 3; AAAAA with quality of TTTSS trimmed to AAA"
        );
        is( $t2, "TTT", "trim quality: quality accordingly trimmed to TTT" );
        is( $c,  1,     "trim quality: trimmed flag set correctly" );
        ...
        # set quality neighborhood to three
        ( $t1, $t2, $c )
            = Utility::trimQuality( "AAAAA", "TTTSS", 20, $t, 3 );
        is( $t1, "AA",
            "trim quality: min quality 20; window size 3; AAAAA with quality of TTTSS trimmed to AA"
        );
        ...
        # set quality neighborhood to three
        ( $t1, $t2, $c )
            = Utility::trimQuality( "AAAAA", "TTUSS", 20, $t, 3 );
        is( $t1, "AAA",
            "trim quality: min quality 20; window size 3; AAAAA with quality of TTUSS trimmed to AAA"
        );
        ...
        ( $t1, $t2, $c )
            = Utility::trimQuality( "AAAAATTTTTCCCCCGGGGGAAAAAT",
            "ZYXWVUTSRQPONMLKJIHGFEDCBA", 20, $t, 1 );
        is( $t1, "AAAAATT",
            "trim quality: sequence trimmed correctly; window size 1" );
        is( $t2, "ZYXWVUT", "trim quality: quality trimmed accordingly" );
        is( $c,  1,         "trim quality: trimmed flag set correctly" );

        ( $t1, $t2, $c )
            = Utility::trimQuality( "AAAAATTTTTCCCCCGGGGGAAAAAT",
            "ZYXWVUTSRQPONMLKJIHGFEDCBA", 20, $t, 3 );
        is( $t1, "AAAAATT",
            "trim quality: sequence trimmed correctly; window size 3" );
        is( $t2, "ZYXWVUT", "trim quality: quality trimmed accordingly" );
        is( $c,  1,         "trim quality: trimmed flag set correctly" );

        ( $t1, $t2, $c )
            = Utility::trimQuality( "AAAAATTTTTCCCCCGGGGGAAAAAT",
            "ZYXWVUTSRQPONMLKJIHGFEDCBA", 20, $t, 5 );
        is( $t1, "AAAAATT",
            "trim quality: sequence trimmed correctly; window size 5" );
        is( $t2, "ZYXWVUT", "trim quality: quality trimmed accordingly" );
        is( $c,  1,         "trim quality: trimmed flag set correctly" );

        ( $t1, $t2, $c )
            = Utility::trimQuality( "AAAAATTTTTCCCCCGGGGGAAAAAT",
            "ZYXWVUTSRQPONMLKJIHGFEDCBA", 20, $t, 7 );
        is( $t1, "AAAAATT",
            "trim quality: sequence trimmed correctly; window size 7" );
        is( $t2, "ZYXWVUT", "trim quality: quality trimmed accordingly" );
        is( $c,  1,         "trim quality: trimmed flag set correctly" );

        # test if average is calculated
        ( $t1, $t2, $c )
            = Utility::trimQuality( "AAAAATTTTT", "UUSUUSSSSS", 20, $t, 3 );
        is( $t1, "AAAAA",
            "trim quality: min quality 20; window size 3; AAAAA with quality of UUSUU trimmed to AAA"
        );
        is( $t2, "UUSUU",
            "trim quality: quality accordingly trimmed to UUSUU" );
        is( $c, 1, "trim quality: trimmed flag set correctly" );
