...
pod2usage( -verbose => 1, -message => "No input file specified" )
    unless -e $input;
pod2usage( -verbose => 1, -message => "No output file specified" )
    unless $output;
...
while ( my $l = <$ifh> ) {
    ...
    if ( $chr ne $activechr ) {
        ...
        die
            "Chromosome $chr occured several times; Please make sure the file is properly sorted"
...

{
    ...
    sub infer_window_size {
        ...
        print
            "Please not that wiggle only allows for non overlapping windows. This window size here will thus correspond to the step-size\n";
        ...
        while (1) {
            ...
            if ( $c1 eq $c2 ) {
                ...
                print
                    "If this is not correct please use the option --window-size\n";
                return $insertsize;
            }
        }
