...
# frame, pileup, chr, start, strand, valid, valid_frame, valid_coverage, valid_codon, count_snps, codon
        my ( $chr, $start, $valid, $count_snps )
            = ( $tr->{chr}, $tr->{start}, $tr->{valid}, $tr->{count_snps} );
        ...
        my ( $sl, $nsl )
            = Utility::calculate_syn_nonsynlength( $tr, $nonsynTable );
        Utility::basicupdate_genelist( $genecollection,
            $_, $chr, $start, $sl, $nsl )
            foreach (@$genelist);
...
#
# synsnplist, nonsynsnplist, synsnps,nonsynsnps,synlen,nonsynlen ,countcodon, coveredreg
        my ( $countcodon, $synlen, $nonsynlen, $synsnps, $nonsynsnps ) = (
            $t->{countcodon}, $t->{synlen}, $t->{nonsynlen},
            $t->{synsnps},    $t->{nonsynsnps}
        );
        ...
        my $synmeasure
            = $vec->calculate_measure( $measure, $t->{synsnplist}, $synlen );
        my $nonsynmeasure
            = $vec->calculate_measure( $measure, $t->{nonsynsnplist},
            $nonsynlen );
        ...
        print $ofh
            "$t->{geneid}\t$nonsynlen\t$synlen\t$nonsynsnps\t$synsnps\t$nonsynmeasure\t$synmeasure\n";
    }
    print "DONE\n";
    close $ofh;
    exit;
