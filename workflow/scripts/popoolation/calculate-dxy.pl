...
my $usage
    = "perl $0 --input mauve-parsing-putput.txt --output dxy-output.txt --window-size 1000 --step-size 100 --min-covered-fraction 0.6\n";
...
if ( $speciesCount != 3 ) {
    ...
    pod2usage(
        -msg     => "Only 3 species are allowed to calculate divergence\n",
        -verbose => 1
    );
}
...
while ( my $window = $reader->nextWindow() ) {
    ...
    if ( $coveredFrac >= $minCoverageFraction ) {
        ...
        my ($string)
            = Utility::calculateDxy( $data, $pos, $chr, $win,
            $minCoverageFraction );
...

{
    ...
    FILL: while ( $line = $self->_nextline ) {
        ...
        my $toret
            = Utility::_annotateWindow( $curwin, $curChr, $self->{lower},
...

{
    ...
    sub testCalculateDxy {
        ...
        while ( my $window = $bpsl->nextWindow() ) {
            ...
            my $dxy = Test::calculateDxy( $data, $pos, $chr, $win,
                $minCoverageFraction, $above );

#print "$dxy->{chr}\t$dxy->{pos}\t$dxy->{considered_pos}\t$dxy->{D12}\t$dxy->{D13}\t$dxy->{D23}\n";

            is( $dxy->{chr}, "2L", "test CalculateDxy, correct chromosome" );
            is( $dxy->{pos}, 5,
                "test CalculateDxy, correct window position" );
            is( $dxy->{considered_pos}, 6,
                "test CalculateDxy, correct number of considered base pairs to calculate Dxy"
            );
            is( $dxy->{D12}, "0.6666666667",
                "test CalculateDxy, the Dxy between species 1 and 2 is correct"
            );
            is( $dxy->{D13}, "0.1666666667",
                "test CalculateDxy, the Dxy between species 1 and 3 is correct"
            );
            is( $dxy->{D23}, "0.8333333333",
                "test CalculateDxy, the Dxy between species 2 and 3 is correct"
            );
    ...
    sub calculateDxy {
        ...
        my ( $data, $pos, $chr, $win, $minCoverageFraction, $above )
        ...
        if ( $coveredFrac >= $minCoverageFraction ) {
            ...
            $string = Utility::calculateDxy( $data, $pos, $chr, $win,
                $minCoverageFraction );
            my ( $chr, $pos, $D12, $D13, $D23, $considered_pos )
                = split( "\t", $string );
            $dxy = {
                chr            => $chr,
                pos            => $pos,
