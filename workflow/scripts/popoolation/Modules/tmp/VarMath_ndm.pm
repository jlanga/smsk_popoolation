...
sub get_D_calculator {
    ...
    return sub {
        ...
        if ( $below == 0 ) {
            warn
                "SNP: at $snp->{chr} $snp->{pos} has almost zero variance! -> Infinite Tajima's D; Details pi-theta: $above; variance: $below; SNP will be ignored\n";
            return 0;
        }
        return 0 if $below == 0;
        $below = sqrt($below);
...
sub get_ddiv_buffer {
    ...
    return sub {
        ...
        for my $m ( $b .. $M - $b ) {
            my $pibpool = $pi_calc->(
                $b, $n, { eucov => $M, C => 0, G => 0, A => $m, T => $M - $m }
            );
            my $thetabpool = $theta_calc->(
                $b, $n, { eucov => $M, C => 0, G => 0, A => $m, T => $M - $m }
            );
