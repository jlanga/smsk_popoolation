{
    ...
    our @ISA = qw(Exporter);
    our @EXPORT
        = qw(load_codon_table load_nonsyn_length_table load_cds_gtf get_codon_changes get_codon_to_gene_assigner);
    ...
    sub load_codon_table {
        ...
        while ( my $l = <$ifh> ) {
            ...
            $codon =~ s/\s+//g;
            $aa    =~ s/\s+//;
            die "Codons must have a length of three bases"
                unless length($codon) == 3;
            die "Codons may only consist of ATCG"
                unless $codon =~ m/^[ATCG]+$/;
        ...
        die
            "Codon table is not complete; must contain 64 entries; provided one only contains: $count_file entries"
            unless $count_file == 64;

        my $count_keys = scalar( keys(%$codonhash) );

        die "One or more codon have been provided multiple times"
            unless $count_keys == 64;
        return $codonhash;
    }
...
#2R	dm3_flyBaseGene	CDS	8375948	8376199	0.000000	-	0	gene_id "CG8824-RC"; transcript_id "CG8824-RC";
            ...
            my ( $chr, $start, $end, $strand, $frame, $attr ) = (
                $ge->{ref},    $ge->{start}, $ge->{end},
                $ge->{strand}, $ge->{frame}, $ge->{attr}
            );
            my ($geneid) = $attr =~ /gene_id\s+"([^"]+)"/;
            die
                "gtf entry $l does not contain an attribute gene_id -> can not infer gene!"
                unless $geneid;
            ...
            foreach my $i ( $start .. $end ) {
                $codonhash->{$chr}{$i}{$genenumber} = 1
                    if ( exists( $codonhash->{$chr}{$i} ) );
            }
        }
        ...
        return sub {
            ...
            my $currentpos = shift;
            die
                "no gene entry found for chromosome $currentchr at position $currentpos"
                unless exists( $codonhash->{$currentchr}{$currentpos} );

            my $genelist = $codonhash->{$currentchr}{$currentpos};
            die "empty gene list for $currentchr at position $currentpos"
                unless @$genelist;
    ...
    sub load_cds_gtf {
        ...
        while ( my $line = <$ifh> ) {
            ...
            # extract the values and check if appropriate
            my ( $chr, $start, $end, $strand, $frame ) = (
                $ge->{ref},    $ge->{start}, $ge->{end},
                $ge->{strand}, $ge->{frame}
            );
            die
                "strand information not correct must be + or -; found instead: $strand"
                unless $strand =~ m/^[-+]$/;
            die
                "frame information not correct must be 0,1 or 2; found instead: $frame"
                unless $frame =~ m/^[012]$/;
            die "start position must be smaller or equal to end position"
                unless $start <= $end;
...
# so if the frame already exists and is in disagreement with the novel provided one we set it to 7 which indicates an invalid frame
    ...
    sub get_codon_changes {
        ...
        my $strand     = $tr->{strand};
        my $start      = $tr->{start};
        die "In order to incorporate a SNP the codon must be valid"
            unless $tr->{valid};
...
# pos, chr, refc, totcov, eucov, A, T, C, G, del, N, iscov, issnp, ispuresnp, alleles (list sorted by most frequent), consc, consc_confidence
            ...
            die "Invalid number of alleles" . Dumper($tr)
                unless ( @$alleles == 4 );

            my $snp_rel_pos = $snp->{pos} - $start;
            $snp_rel_pos = 2 - $snp_rel_pos
                if $strand eq
                "-";    # for the reverse stand the whole thing is reversed

            my $rc     = substr( $codon, $snp_rel_pos, 1 );
            my $alhash = _get_major_alleles( $snp->{alleles}, $strand );
            die "character of codon must be part of the two major alleles"
                unless exists( $alhash->{$rc} );
            ...
            die "codon with snp is the same as codon without snps"
                if $modcodon eq $codon;
