{
    ...
    our @ISA = qw(Exporter);
    our @EXPORT
        = qw(get_basic_parser get_pileup_parser get_extended_parser get_empty_pileup_entry get_quality_encoder get_basic_mpileupparser);
    ...
    sub get_basic_mpileupparser {
        ...
        die "Encoder $quality_encoding not supported"
            unless exists( $qualhash->{$quality_encoding} );
...
#    chrM    1       G       76      ^F.^F.^F.^F.    hhhgf    228     ^F,^F,^F,^F,^F.  ghfhhhhh    157     F,^F,^F,^F.^F, gYheghadh
    ...
    sub _get_subnucsqualparser {
        ...
        return sub {
            ...
            $nucs =~ s/[-+](\d+)(??{"[ACGTNacgtn]{$1}"})//g
                ; # I offer a beer for anyone who understand this line, I am a genius!!!!
            $nucs =~ s/\^.//g;
            $nucs =~ s/\$//g;
            $nucs =~ s/[.]/uc($rc)/eg;
            $nucs =~ s/[,]/lc($rc)/eg;
            ...
            die
                "size of sequence does not equal size of quality:  $nucs, $qual\n"
...
#X        2832683 N       22      *AaAAAaaaAaaaaaaaaaaA^FA        yBaB_\aaa^a[a^____^a[^
    ...
    sub get_pileup_parser {
        ...
        return sub {
            ...
            $pu->{iscov} = 0;
            $pu->{issnp} = 0;
            if (   $pu->{eucov} >= $mincoverage
                && $pu->{eucov} <= $maxcoverage )
            {
                $pu->{iscov} = 1;
                my $alcount = 0;
                ...
                $pu->{ispuresnp} = 0;
                if ( $pu->{issnp}
                    and ( $pu->{del} < 1 or $tolerateDeletions ) )
                {
                    $pu->{ispuresnp} = 1;
                }
  ...
  # pos, chr, refc, totcov, eucov, A, T, C, G, del, N, iscov, issnp, ispuresnp
    ...
    sub get_extended_parser {
        ...
        # qualencoding,mincount,mincov,maxcov,minqual
        my $pp
            = get_pileup_parser( $quality_encoding, $mincount, $mincoverage,
            $maxcoverage, $minqual, $tolerateDeletions );
...
# pos, chr, refc, totcov, eucov, A, T, C, G, del, N, iscov, issnp, ispuresnp, alleles (list sorted by most frequent), consc, consc_confidence
            die "invalid number of allles for entry $line"
                unless scalar( @{ $pu->{alleles} } ) == 4;
            return $pu;
        }
    }
