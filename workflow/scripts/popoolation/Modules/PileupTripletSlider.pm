{
    ...
    sub _next_codon {
        ...
        $t->{frame} = [
            $annotation->{$chr}{$pos},
            $annotation->{$chr}{ $pos + 1 },
            $annotation->{$chr}{ $pos + 2 }
        ];
...
# step two: store the first line of the pileup and read the next two lines of the pileup
        ...
        my $triplet = [
            $p1,
            get_empty_pileup_entry( $chr, $pos + 1 ),
            get_empty_pileup_entry( $chr, $pos + 2 )
        ];
        while (1) {
            my $l = $self->_nextline();
            last unless $l;
    ...
    sub _annotate_triplet {
        ...
        for ( my $i = 0; $i <= 2; $i++ ) {
            my $act = $pileups->[$i];
            die "pileup triplet is fucked, chromsomes do not fit"
                unless $act->{chr} eq $chr;
            die "pileup triplet is fucked, positions are not ascending"
                unless ( $start + $i == $act->{pos} );
        }
...
# consc_confidence -> is the SNP AAATTT or AAAATTT -> in the second the consensus basecall is clearly A
            $validConsensus = 0
                unless $p->{consc_confidence} >= $consc_confidence_threshold;
        ...
        # is valid
        my $valid
            = ( $validCoverage and $validCodon and $validConsensus ) ? 1 : 0;
