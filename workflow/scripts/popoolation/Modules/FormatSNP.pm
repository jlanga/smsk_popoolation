{
    ...
    our @ISA = qw(Exporter);
    our @EXPORT
        = qw(get_WindowSNPFormater get_gtfSNPFormater get_SNPwriter get_syn_nonsyn_SNPFormater get_synnonsyngene_SNPFormater);
    ...
    sub _formatSNP {
        my $snp = shift;
        return
            "$snp->{chr}\t$snp->{pos}\t$snp->{refc}\t$snp->{eucov}\t$snp->{A}\t$snp->{T}\t$snp->{C}\t$snp->{G}\t$snp->{N}\n";
    }
    ...
    sub _format_cc {
        ...
        my $toprint
            = "$cc->{chr}\t$cc->{pos}\t$cc->{ref}\t$cc->{eucov}\t$cc->{A}\t$cc->{T}\t$cc->{C}\t$cc->{G}\t$syn\t$strand\t$codon\t$aa";
        return $toprint;
    }
}
