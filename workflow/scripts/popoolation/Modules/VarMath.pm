...
our @ISA = qw(Exporter);
our @EXPORT
    = qw(noverk get_theta_calculator get_pi_calculator get_D_calculator);
our @EXPORT_OK
    = qw(get_thetadiv_buffer get_thetadiv_buffer_sqr get_nbase_buffer);
...
sub get_ddivisor {
    ...
    return sub {
        ...
        my $div
            = ( $alphastar / $snpcount ) * $theta + $betastar * ( $theta**2 );
        return sqrt($div);
    }
}
...
sub get_alphastar_calculator {
    ...
    return sub {
        my $n = shift;
        die "invalid effective coverage; has to be larger than 1"
            unless $n > 1;
...
#            r=1
{
    ...
    return sub {
        ...
        my $key = "$M:$b:$n";
        return $thetadiv_buffer_sqr->{$key}
            if exists( $thetadiv_buffer_sqr->{$key} );
