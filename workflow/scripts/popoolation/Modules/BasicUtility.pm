{
    ...
    our @ISA = qw(Exporter);
    our @EXPORT
        = qw(reverse_complement_dna load_fasta get_fasta_reader get_fasta_writer get_fastq_reader get_fastq_writer);
    ...
    sub load_fasta {
        ...
        while ( my $l = <$ifh> ) {
            ...
            if ( $l =~ m/^>/ ) {
                if ($header) {
                    die "Reference $header already exists"
                        if exists( $refhash->{$header} );
                    $refhash->{$header} = $seq;
                }
                ($header) = $l =~ m/^>(\S+)/;
                $seq = "";
