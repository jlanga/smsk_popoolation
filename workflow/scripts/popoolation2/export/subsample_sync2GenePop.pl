...
for my $i ( 0 .. ( $popcount - 1 ) ) {
    ...
    die
        "User specified region does not contain any SNPs, can not create GenePop output for no SNPs"
        unless @$popar;
...

{
    ...
    sub print_genepop {
        ...
        print $ofh
            "Haploid genotypes for region $region, subsampled to a coverage of $coverage; NOTE: haplotype structure is a mere artifact!"
            . $endl;
