...
pod2usage(
    -msg     => "Indel window has to be larger or equal to one",
    -verbose => 1
) unless $indelwindow >= 1;
...
print
    "How many bp of the reference are covered by indel-regions: $count_bases_covered\n";
...
# AB000381 Twinscan  CDS          700   707   .   +   2  gene_id "AB000381.000"; transcript_id "AB000381.000.1";
        my $str
            = "$e->{chr}\tmpileup\tindelregion\t$e->{start}\t$e->{end}\t.\t.\t.\tgene_id \".\"; transcript_id \".\";\n";
    }
    ...
    sub get_region_calculator {
        ...
        return sub {
            ...
            while (@$indels) {
                ...
                if ( $next->{chr} eq $chr
                    && ( $activepos + 2 * $windowlength ) >= $next->{pos} )
                {
...

{
    ...
    sub test_parse_pileup {
       ...
       # + .. insertion in the read (no size)
        ...
        ( $valid, $best )
            = Utility::parse_pileup( "..-3ACG.-1A...-2AA..", 1 );
...
perl identify-indel-regions.pl - A script which identifies the surrounding indels and converts these coordindates into a gtf-file

=head1 SYNOPSIS

perl identify-indel-regions.pl --input input.mpileup --output indel-regions.gtf --indel-window 5
...
The input file in the mpileup format; Mandatory.
...
This script identifies indels from a mpileup file; It furthermore converts the coordinates of the positions around the indels into a gtf-formated file;
As a typical mpileup file contains several samples, indels are identfied across all samples.
This file can for example be used to filter a synchronized file (filter-sync-by-gtf.pl)

=head2 Input pileup

A pileup file as described here: http://samtools.sourceforge.net/pileup.shtml; example:

 2L	90131	N	11	AaAAAaaAaAA	[aUQ_a`^_\Z     11	AaAAAaaAaAA	[aUQ_a`^_\Z
 2L	90132	N	11	AaAAAa-1NaAaAA	_bYQ_^aaT^b     11	AaAAAaaAaAA	[aUQ_a`^_\Z
 2L	90133	N	11	A$aAAAaaAaAA	_b[Xaaa__Ua     11	AaAAAaaAaAA	[aUQ_a`^_\Z
 2L	90134	N	10	tTTTttTtTT	_`aaa_a[aa      11	AaAAA-2NNaaAaAA	[aUQ_a`^_\Z
 2L	90135	N	10	a$TAAaa-1TAaAA	aZ^a`ba`\_      10	a$TAAaaAaAA	aZ^a`ba`\_

=head2 Output

 A gtf file specifying the regions around indels
