{
    ...
    sub test_withreplacement {
        ...
        is( $r->{A}, 10,
            "Test subsampler with replacment, A count is correct" );
        is( $r->{T}, 0,
            "Test subsampler with replacment, T count is correct" );
        is( $r->{C}, 0,
            "Test subsampler with replacment, C count is correct" );
        is( $r->{G}, 0,
            "Test subsampler with replacment, G count is correct" );
    ...
    sub test_withoutreplacement {
        ...
        $r  = $ss->( { A => 20, T => 0, C => 0, G => 0, N => 0, del => 0 } );
        is( $r->{A}, 10,
            "Test subsampler without replacment, A count is correct" );
        is( $r->{T}, 0,
            "Test subsampler without replacment, T count is correct" );
        is( $r->{C}, 0,
            "Test subsampler without replacment, C count is correct" );
        is( $r->{G}, 0,
            "Test subsampler without replacment, G count is correct" );
        $r = $ss->( { A => 0, T => 20, C => 0, G => 0, N => 0, del => 0 } );
        is( $r->{A}, 0,
            "Test subsampler without replacment, A count is correct" );
        is( $r->{T}, 10,
