        ...
        return sub {
            ...
            for ( my $i = 0; $i < @a; $i++ ) {
                ...
                if ( $col =~ /-/ ) {
                    ...
                    my @parts = split /:/, $col;
                    die
                        "failed parsing $col; does not have the correct number of entries"
                        unless @parts == 6;
                    $e = {
                        A   => $parts[0],
                        T   => $parts[1],
...
# also reset the general appropriate coverage unless every entry has the correct coverage;
            ...
            $issnp = 0
                unless $is_suficient_covered
                ; # no SNP will ever be allowed at a position which is not sufficiently covered in all data files!!
