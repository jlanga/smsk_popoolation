{
    ...
    sub _withoutreplace {
        my $p         = shift;
        my $targetcov = shift;
        my ( $ac, $tc, $cc, $gc, $nc, $dc )
            = ( $p->{A}, $p->{T}, $p->{C}, $p->{G}, $p->{N}, $p->{del} );
    ...
    sub _withreplace {
        ...
        my ( $af, $tf, $cf, $gf, $nf, $df ) = (
            $ac / $cov, $tc / $cov, $cc / $cov,
            $gc / $cov, $nc / $cov, $dc / $cov
        );
        my $ab = $af;
        my $tb = $af + $tf;
        my $cb = $af + $tf + $cf;
...
# ingenious, first iteratively increase the coverage until the target is reached, if coverage than is to high iteratively decrease it until targetcoveage is reached
        ...
        my $nuevocov
            ...
            + $tr->{N}
            + $tr->{del};
        die "calculated coverage is larger than the targetcoverage"
            if $nuevocov > $targetcov;
